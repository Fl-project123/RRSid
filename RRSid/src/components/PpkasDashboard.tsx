...
export default function PpkasDashboard({
  ...
  const [subRole, setSubRole] = useState<string>('Dispatcher Terpusat');
  ...
  const checkActionPermission = (type: 'signal' | 'switch' | 'pjl', itemZone: number): boolean => {
    // 1. Check zone permission matching selectedZone
                ...
                <button
                  ...
                  className={`p-3.5 rounded-xl border text-left flex flex-col justify-between transition-all cursor-pointer ${
                    selectedZone === zone.id
                      ...
                      : 'bg-[#0e1322] border-slate-900 text-slate-400 hover:border-indigo-950/80 hover:bg-[#0f1528]'
                  }`}
                >
              ...
              <div className="bg-[#060a14] rounded-lg border border-slate-900 p-2 flex-1 relative overflow-y-auto flex flex-col justify-start items-center min-h-[220px]">
                <GameCanvas
                  ...
                  onSwitchToggle={(switchId, state) => {
                    ...
                    if (!sw) return;
                    if (!checkActionPermission('switch', sw.zone)) return;
                    onToggleSwitch(switchId, state);
                  }}
                  onSignalToggle={(sigId) => {
                    const sig = signals.find(s => s.id === sigId);
                    if (!sig) return;
                    if (!checkActionPermission('signal', sig.zone)) return;
                  ...
                  {filteredPjls.map(p => {
                    ...
                    return (
                      <g key={p.id} className="cursor-pointer" onClick={() => {
                        if (!checkActionPermission('pjl', p.zone)) return;
