...
export default function RealWorldMap({
  ...
  // Handle auto-pan to active zone in PPKA dispatcher dashboard
  useEffect(() => {
    if (!mapRef.current || !isMapReady || !selectedZone) return;

    // Define center KPs for specific zones
    let targetZoneKp = 10.0; // Zone 1 center (Manggarai around KM 10)
    if (selectedZone === 2) targetZoneKp = 31.0; // Zone 2 center (Depok/Citayam around KM 31)
    else if (selectedZone === 3) targetZoneKp = 48.0; // Zone 3 center (Bogor around KM 48)
  ...
  useEffect(() => {
    ...
    trains.forEach(tr => {
      ...
      coachIndices.forEach((idx) => {
        ...
        if (idx === 0) {
          coords = getLatLngFromKp(tr.kp, tr.direction);
          bearing = getBearingAtKp(tr.kp, tr.direction);
        } else {
          ...
          let lastCoords = getLatLngFromKp(tr.kp, tr.direction);
          
          const step = 0.0012; // step of about 0.8 meters
          const maxIterations = 80;
          let iterations = 0;
          
          coords = getLatLngFromKp(tr.kp, tr.direction); // pre-fallback
          bearing = getBearingAtKp(tr.kp, tr.direction); // pre-fallback
          ...
          while (accumulatedDist < targetDist && iterations < maxIterations) {
            ...
            const nextCoords = getLatLngFromKp(currentKp, tr.direction);
            ...
            if (accumulatedDist >= targetDist) {
              ...
              bearing = getBearingAtKp(currentKp, tr.direction);
              break;
            }
